...
_task_switch
    ...
    MRS     r3, PSP
    ; Save floating-point status and control register, if FPU is in use
    TST     lr, #0x10
    IT      EQ
    VMRSEQ 	r12, FPSCR
    STMDBEQ r3!, {r12}
    ; Save upper half of floating-point registers, if FPU is in use
    VSTMDBEQ r3!, {s16-s31}
    ...
    LDMFD   r3!, {r4-r11}
    ; Restore upper half of floating-point registers, if FPU was in use
    VLDMIAEQ r3!, {s16-s31}
    ; Restore floating-point status and control register, if FPU was in use
    LDMIAEQ r3!, {r12}
    IT      EQ
    VMSREQ  FPSCR, r12
